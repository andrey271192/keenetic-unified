...
LOG="/opt/var/log/hydra_update.log"
# Auto-detect HydraRoute config directory (HydraRoute= or hydra=)
if [ -d "/opt/etc/HydraRoute" ]; then HR_DIR="/opt/etc/HydraRoute"
elif [ -d "/opt/etc/hydra" ]; then HR_DIR="/opt/etc/hydra"
else HR_DIR="/opt/etc/HydraRoute"; fi
DC="$HR_DIR/domain.conf"; IL="$HR_DIR/ip.list"; VF="$HR_DIR/server_version"
log() { echo "$(date '+%Y-%m-%d %H:%M:%S') $1" >> "$LOG"; }
[ -z "$SERVER" ] && exit 1; CV=$(cat "$VF" 2>/dev/null || echo "none"); NV=$(curl -sf --connect-timeout 10 "$SERVER/api/hydra/version" 2>/dev/null)
[ -z "$NV" ] && exit 0; [ "$CV" = "$NV" ] && log "[SKIP] уже v$NV" && exit 0
log "[UPDATE] $CV → $NV"; mkdir -p "$HR_DIR"; cp "$DC" "${DC}.bak" 2>/dev/null; cp "$IL" "${IL}.bak" 2>/dev/null
